...
public sealed class CliRunner
{
    ...
    private ToolResult Execute(CliOptions options, Action<ExportProgress> progress)
    {
        return options.Command switch
        {
            ...
            "export-sheet" => _cardToolService.ExportSheet(options.DeckId, options.OutputPath, options.Paper, options.Dpi, options.BackMirror, options.IncludeMeasurementGuide, progress, options.EasyPrintBacks, options.PrintCompensationPercent),
            "export-diy" => _cardToolService.ExportDiy(options.DeckId, options.OutputPath, options.Dpi, options.BackMirror, options.IncludeMeasurementGuide, progress, options.PrintCompensationPercent),
    ...
    private static string GetHelpText()
    {
        ...
        Print sheet options:
          ...
          --print-compensation <90-110>  Uniformly compensate card, bleed, placement, and guide size (default: 100).
